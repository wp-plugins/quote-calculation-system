...
function register_quote_settings() {
	...
	//for paypal
	register_setting( 'quote-settings-group', 'paypal-email-address' );
	register_setting( 'quote-settings-group', 'paypal-currency-code' );
	register_setting( 'quote-settings-group', 'paypal-success-return' );
	register_setting( 'quote-settings-group', 'paypal-cancel-return' );
}
function quote_system_extra_option() {
	...
	$paypalEmailAddress = get_option('paypal-email-address');
	$paypalCurrencyCode = get_option('paypal-currency-code');
	$paypalSuccessReturn = get_option('paypal-success-return'); 
	$paypalCancelReturn = get_option('paypal-cancel-return');
	?>
	<div class="wrap">
	...
	<form method="post" action="options.php">
		...
		<table class="form-table">
			...
			<tr><th><h2>Paypal Setting</h2></th></tr>
			<tr valign="top">
			<th scope="row">Paypal Email Address</th>
				<td><input type="text" name="paypal-email-address" id="paypal-email-address" class="regular-text" value="<?php echo $paypalEmailAddress?>" /></td>
			</tr>
			<tr valign="top">
			<th scope="row">Paypal Currency Code </th>
				<td><input type="text" name="paypal-currency-code" id="paypal-currency-code" class="small-text" value="<?php echo $paypalCurrencyCode?>" />
					<em><small>(Stardard Currency Code as defined by paypal as USD for US $)</small></em>
				</td>
			</tr>
			<tr valign="top">
			<th scope="row">Paypal Success Return URL </th>
				<td><input type="text" name="paypal-success-return" id="paypal-success-return" class="regular-text" value="<?php echo $paypalSuccessReturn?>" />
					<em><small>(Stardard Currency Code as defined by paypal as USD for US $)</small></em>
				</td>
			</tr>
			<tr valign="top">
			<th scope="row">Paypal Cancel Return URL </th>
				<td><input type="text" name="paypal-cancel-return" id="paypal-cancel-return" class="regular-text" value="<?php echo $paypalCancelReturn?>" />
					<em><small>(Stardard Currency Code as defined by paypal as USD for US $)</small></em>
				</td>
			</tr>
		</table>
...
if (isset($_POST['front_submit_location_based'])) {
	...
	if($captcha == $confirm_captcha) {
		...
		if ($trueBool =mail($to,$subject,$message,$headers)) {
			echo "Message Sucessfully Submitted. You will be redirected to paypal for payment."
			?>
			<form action="https://www.paypal.com/cgi-bin/webscr" method="post" target="_top" name="paypalpayment">
				<input type="hidden" name="cmd" value="_xclick">
				<input type="hidden" name="business" value="<?php echo get_option('paypal-email-address')?>">
				<input type="hidden" name="lc" value="US">
				<input type="hidden" name="item_name" value="<?php echo ' from '.$from_location .' to '. $to_location;?>">
				<input type="hidden" name="item_number" value="<?php echo 'Vehicle-'.$type_of_vehicle;?>">
				<?php if (isset($total_amounts)) { ?>
				<input type="hidden" name="amount" value="<?php echo $total_amounts;?>">
				<?php } else {?>
				<input type="hidden" name="amount" value="<?php echo $rate;?>">
				<?php }?>
				<input type="hidden" name="currency_code" value="<?php echo get_option('paypal-currency-code')?>">
				<input type="hidden" name="button_subtype" value="services">
				<input type="hidden" name="no_note" value="0">
				
				<input type="hidden" name="return" value="<?php echo get_option('paypal-success-return');?>">
				<input type="hidden" name="cancel_return" value="<?php echo get_option('paypal-cancel-return')?>">				
			</form>
			<script type="text/javascript">
